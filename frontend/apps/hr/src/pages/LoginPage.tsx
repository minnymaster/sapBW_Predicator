...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (payload.role !== 'hr') {
        setError('Доступ разрешён только для HR-менеджеров.');
        return;
      }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Портал HR</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-8 py-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-violet-600 text-white text-sm font-medium rounded-lg
